  ...
  render() {
      ...
      return (
        <div className="container-sleep">
          <div className="exclamation">
            <FontAwesomeIcon icon={faBed} size="3x" color="#cc0001" />
          </div>
    ...
    if (this.props.vehicle && this.props.vehicle.state === 'offline') {
      return (
        <div className="container-sleep">
          <div className="exclamation">
            <FontAwesomeIcon icon={faPowerOff} size="3x" color="#cc0001" />
          </div>
    ...
    return (
      <div>
        <Modal
          visible={this.state.climateTempShowModal}
          onOk={this.handleClimateTempOk}
          onCancel={this.handleClimateTempCancel}
          errorMessage={this.state.climateTempModalError}
          title="Desired temperature"
          info="Set temperature for both passenger and driver"
          type="number"
          placeholder={`Temperature in ${this.props.vehicle.temperatureUnits}`}
        />
        <div className="car-model">
          <div className="title">Tesla {this.props.vehicle.model}</div>
          <ReactTooltip place="bottom" id="overview">
            <div className="tooltip">
              Car version: <span>{this.props.status.carVersion}</span>
            </div>
            <div className="tooltip">
              Charging state: <span>{this.props.status.chargingState}</span>
            </div>
            <div className="tooltip">
              Battery level: <span>{`${this.props.status.batteryLevel}%`}</span>
            </div>
            <div className="tooltip">
              Time to full charge:{' '}
              <span>{`${this.props.status.timetoFullCharge} hours`}</span>
            </div>
            <div className="tooltip">
              Door:{' '}
              <span>{this.props.status.locked ? 'Locked' : 'Unlocked'}</span>
            </div>
            <div className="tooltip">
              Climate: <span>{this.props.status.climate ? 'ON' : 'OFF'}</span>
            </div>
            <div className="tooltip">
            ...
            <div className="tooltip">
              Sentry mode:{' '}
              <span>{this.props.status.sentryMode ? 'ON' : 'OFF'}</span>
            </div>
            <div className="tooltip">
              Valet mode:{' '}
              <span>{this.props.status.valetMode ? 'ON' : 'OFF'}</span>
            </div>
          </ReactTooltip>
          <a data-tip data-for="overview">
            <img
              src={modelImage(this.props.vehicle.model)}
              width="100"
              height="40"
            />
          </a>
        ...
        <div className="status">
          ...
          <div>
            <span className="description">
              <FontAwesomeIcon
                name={this.props.status.chargingState}
                icon={this.props.batteryIcon.type}
                size="1x"
                color={this.props.batteryIcon.color}
              />
              <strong>
                {' '}
                {this.props.vehicle.distanceUnits.split('/')[0] === 'km'
        ...
        <div className="controls">
          ...
          <div className="controls-items">
            <Actions
              ...
              handle={this.handleLockClick}
              status={this.props.status.locked}
            />
            <Actions
              type="climate"
              loading={this.props.actionLoading}
              handle={this.handleFanClick}
              status={this.props.status.climate}
            />
            {this.props.status.sentryModeAvailable ? (
              <Actions
                type="sentryMode"
                loading={this.props.actionLoading}
                handle={this.handleSentryClick}
                status={this.props.status.sentryMode}
              />
            ) : null}
            <Actions
              type="climateTemp"
              loading={this.props.actionLoading}
              handle={this.handleClimateTempClick}
              status={this.props.status.driverTempSetting}
            />
          </div>
        </div>
        <hr />
        <div className="location">
          <div className="summary">
            Location{' '}
            <a onClick={this.handleOpenMapClick}>
              <FontAwesomeIcon
